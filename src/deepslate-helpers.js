...
function loadDeepslateResources(textureImage) {
  ...
  const normalizeId = id => id?.startsWith('minecraft:') ? id.slice(10) : id;

  const addModelId = id => {
    id = normalizeId(id);
    if (!id || modelIds.has(id)) return;
    if (!assets.models[id]) return;
    modelIds.add(id);
    const parent = assets.models[id].parent;
    if (parent && !parent.startsWith('builtin/')) {
      addModelId(parent);
    }
  };

  Object.values(assets.blockstates).forEach(state => {
    if (state.variants) {
      Object.values(state.variants).forEach(variant => {
        if (Array.isArray(variant)) {
          variant.forEach(v => addModelId(v.model));
        } else if (variant && typeof variant === 'object') {
          if (variant.model) addModelId(variant.model);
          if (variant.models) variant.models.forEach(v => addModelId(v.model));
        }
      });
    }
    if (state.multipart) {
      state.multipart.forEach(entry => {
        if (entry.apply) {
          if (entry.apply.model) addModelId(entry.apply.model);
          if (entry.apply.models) entry.apply.models.forEach(v => addModelId(v.model));
        }
      });
    }
  });
